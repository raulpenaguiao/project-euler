...
for i in range(56, 50000*6+1):
    S.append((S[i-24]+S[i-55])%MOD)
C = [0] + [[[S[6*i-5]%Mod, S[6*i-4]%Mod, S[6*i-3]%Mod], [1 + S[6*i-2]%mod, 1 + S[6*i-1]%mod, 1 + S[6*i]%mod]] for i in range(1, LIM+1)]
print(C[55])
splits = 20
splitbounds = [math.floor(i*Mod/splits) + 0.5 for i in range(splits+1)]
splitbounds[0] = -0.5
...
neigs = [Set(elements = []) for _ in range(LIM+1)]
for e in edges:
...
while True:
    ...
    for p in prevcliques:
        ...
        #Can we enlarge this clique? We will enlarge exclusively with a cube intersection that is smaller
        #This ensures that we are counting each clique only once
        m = min(p[0])
        for v in p[2].bag:
            if v < m:
